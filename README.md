...
- **案例匹配** - 根因相似度 + 语义匹配
- **动态生成** - 个性化响应，而非固定模板
- **详细数据支持** - 优先使用实际采集数据（翻台率、NPS等）⭐v3.1新增
- **智能可视化** - 自动生成彩色卡片和图表 ⭐v3.1新增

**技术架构**:
```
Query → LLM结构化 → Intent分类 →
诊断推理/数据聚合/对比分析 → LLM动态生成响应 → 智能可视化
```
...
### v3.1-dev (2026-02-11) - Current ✅
**商户详细运营数据扩展 + AI助手增强 + 数据可视化**
- ✅ 商户详细运营数据采集和展示（7大类别，30+字段）
- ✅ AI助手图标升级（尺寸+28%，蓝紫渐变，多重动画）
- ✅ AI响应格式优化（去除Markdown，智能可视化）
- ✅ 聚合查询支持（基础统计、平均值计算、对比分析）
- ✅ 3大数据可视化功能（关键指标预警、运营趋势图、同业对比图）
- 📋 新增代码: ~2000行，17 commits

### v3.0-dev (2026-02-07~02-11) ✅
**AI问答助手系统性重构**
...
**Version**: v3.1-dev
**Status**: Completed ✅ (商户详细运营数据 + AI助手增强 + 数据可视化)
