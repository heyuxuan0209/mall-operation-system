...
#### 1.1 AI智能问答Agent（因果推理诊断）⭐v3.0+v3.1核心升级
**技术定位**：因果推理诊断系统（类GraphRAG架构）

**核心问题**：
- 用户反馈"答非所问、僵化、千篇一律"
- Dashboard只能展示数据，无法回答"为什么"
- 无法处理聚合统计、对比分析等复杂查询

**技术方案**：
- **结构化查询理解**：LLM驱动的Prompt工程（3层设计：任务定义+Few-shot示例+Schema约束），将自然语言转为JSON结构
- **多跳推理**：Hop1提取商户指标 → Hop2关联分析 → Hop3因果推理，构建完整证据链
- **置信度计算**：LLM根据数据质量判断（真实数据85-95%，健康度指标推断70-85%，完全推测50-70%），规则验证防止幻觉
- **错误处理**：3层防护（LLM响应解析+规则验证修正+降级机制），确保系统鲁棒性

**核心差异**（vs 传统RAG）：
- 传统RAG：向量检索 → 相似案例 → 泛泛而谈
- 因果推理：结构化解析 → 多跳推理 → 根因+证据链+置信度

**实际案例**：
- 查询："海底捞为什么营收低？"
- 输出："位置劣势（主因，90%置信度）→ 3层客流比1-2层低40% → 翻台率1.2次/天（行业标准3.0次）→ 营收低"

**价值**：
- 查询理解准确率：60% → >90%（⬆50%）
- 晨会数据准备：2小时 → 5分钟（⬇96%）
- 诊断深度：从"营收低"（症状）→ 完整根因+证据链
- 决策效率：30分钟人工分析 → 3秒AI诊断（⬆600倍）

**技术细节**：详见 `docs/career/AI-QA-AGENT-TECH-DEEP-DIVE.md`

---

#### 1.1.1 Prompt工程深度解析 ⭐核心技术能力

**什么是Prompt工程？**

Prompt工程是设计和优化LLM输入指令的系统化方法，目标是让LLM准确理解任务意图、遵循输出格式、提供可靠结果。在AI智能问答Agent中，Prompt工程是连接"用户自然语言"和"系统结构化处理"的关键桥梁。

**为什么需要Prompt工程？（业务视角）**

| 问题场景 | 没有Prompt工程 | 有Prompt工程 | 业务价值 |
|---------|--------------|------------|---------|
| **用户问"这个月几家高风险商户"** | LLM返回："大约有一些高风险商户需要关注" | LLM返回：`{type: "aggregation", filters: {riskLevel: ["high"]}, operation: "count"}` | 可执行的结构化查询 |
| **用户问"小龙坎呢？"（省略主语）** | LLM不理解上下文，返回："请问您想了解小龙坎的什么信息？" | LLM从上下文继承商户，返回：`{merchants: ["小龙坎"], intent: "health_query"}` | 支持自然对话流 |
| **LLM返回格式不一致** | 有时返回JSON，有时返回文本解释 | 严格返回JSON Schema | 系统可靠性100% |

**核心痛点：**
- 运营人员不会写SQL，只会说"这个月多少高风险商户"
- 需要AI理解自然语言，转为系统可执行的查询
- 需要处理follow-up问题（"小龙坎呢？"）
- 需要确保LLM输出格式一致，避免系统崩溃

---

**Prompt工程的3层设计架构**

##### 第1层：任务定义 + 上下文注入（解决"理解用户意图"问题）

**业务场景：**
用户在查询"海底捞怎么样"后，接着问"小龙坎呢？"。如果LLM没有上下文，会不理解"呢"指的是什么。
...
# 对话上下文
- 最近讨论商户：海底捞
- 上一轮意图：health_query（健康度查询）
- 最近消息：海底捞怎么样 / 有风险吗 / 小龙坎呢？

# 上下文理解规则
如果用户输入省略了主语（使用"它"、"这家店"、"呢"等），需要从上下文补全。
- "小龙坎呢？" → 应理解为"小龙坎的健康度怎么样？"
```

**产品思维：**
- 用户不会每次都说完整句子，需要支持自然对话流
- 上下文注入让AI"记住"之前的对话，像人一样理解省略表达
- 这是从"工具型AI"到"对话型AI"的关键

**调优过程：**
1. **v1.0问题**：用户问"小龙坎呢？"，LLM返回："请问您想了解小龙坎的什么信息？"
2. **分析**：LLM没有上下文，不知道"呢"指的是什么
3. **v2.0优化**：增加"最近讨论商户"和"上一轮意图"
4. **结果**：LLM正确理解为"小龙坎的健康度怎么样"，准确率从40% → 85%

---

##### 第2层：规则定义 + Few-shot示例（解决"输出格式一致性"问题）

**业务场景：**
用户问"这个月多少高风险商户"，系统需要识别这是"聚合统计查询"，而不是"单商户查询"。
...
- "平均健康度是多少" → {type: "aggregation", aggregations: {operation: "avg", field: "healthScore"}}

## 2. comparison (对比查询)
**触发条件**：包含"对比"、"比较"、"vs"、"和...相比"
**示例**：
- "海底捞和小龙坎对比" → {type: "comparison", merchants: ["海底捞", "小龙坎"], comparisonTarget: "merchant_vs_merchant"}
- "海底捞和上月对比" → {type: "comparison", merchants: ["海底捞"], comparisonTarget: "last_month"}

## 3. single_merchant (单商户查询)
**触发条件**：提到具体商户名，问健康度、风险、帮扶等
**示例**：
- "海底捞怎么样" → {type: "single_merchant", merchants: ["海底捞"], intents: ["health_query"]}
```

**产品思维：**
- Few-shot Learning（少样本学习）：通过示例教LLM如何解析，比纯指令更准确
- 触发条件明确：减少LLM的模糊判断，提高一致性
- 分类清晰：4种查询类型（single_merchant, aggregation, comparison, trend_analysis）

**调优过程：**
1. **v1.0问题**：用户问"多少高风险商户"，LLM有时返回aggregation，有时返回single_merchant
2. **分析**：LLM对"多少"的理解不稳定，需要明确规则
3. **v2.0优化**：增加触发条件（"多少"、"几个"、"数量"）+ 3个示例
4. **结果**：聚合查询识别准确率从70% → 95%
...
##### 第3层：Schema约束 + 关键约束（解决"LLM幻觉"问题）

**业务场景：**
LLM可能返回错误格式，比如把"对比海底捞和小龙坎"解析为`merchants: ["对比", "海底捞", "小龙坎"]`（把"对比"也当成商户名）。
...
# 输出格式 (严格JSON，不要有任何额外文字)
{
  ...
  "entities": {
    "merchants": ["商户名1", "商户名2"],  // 字符串数组，不是对象数组
...
- 关键约束：针对实际测试中发现的LLM常见错误（如把"对比"也放进merchants数组）
- 严格JSON：避免LLM返回"好的，我来解析..."这种废话

**调优过程：**
1. **v1.0问题**：用户问"对比海底捞和小龙坎"，LLM返回：`merchants: ["对比", "海底捞", "小龙坎"]`
2. **分析**：LLM把"对比"也当成了商户名
3. **v2.0优化**：增加关键约束："merchants数组只包含商户名称，不要包含'对比'等关键词"
4. **结果**：商户名提取准确率从80% → 98%

**实际案例：**
- 用户输入："对比海底捞和小龙坎"
- v1.0错误输出：`merchants: ["对比", "海底捞", "小龙坎"]`
- v2.0正确输出：`merchants: ["海底捞", "小龙坎"]`
...
**方法：**
- 在测试环境让运营人员真实使用，记录所有查询
- 标注哪些查询被正确理解，哪些被误解
- 分类Badcase：格式错误、意图识别错误、实体提取错误

**实际数据：**
- 测试100个查询，发现25个Badcase
- 分类：聚合查询误判（10个）、商户名提取错误（8个）、上下文理解失败（7个）

##### 步骤2：分析根因

**聚合查询误判案例：**
- 用户："几家高风险商户"
- LLM返回：`type: "single_merchant"`（错误）
- 根因：LLM对"几家"的理解不稳定

**解决方案：**
- 增加触发条件："几家"、"几个"、"几"
- 增加示例："几家高风险商户" → aggregation

##### 步骤3：A/B测试验证

**测试方法：**
- v1.0 Prompt（无触发条件）vs v2.0 Prompt（有触发条件）
- 各测试50个查询，对比准确率
...
**告警机制：**
- 准确率<85% → 触发告警，人工review Badcase
- 格式错误率>10% → 检查Schema约束是否失效
...
|------|--------|--------|------|
| **查询理解准确率** | 60% | >90% | ⬆50% |
| **支持查询类型** | 1种（单商户） | 4种（单商户+聚合+对比+趋势） | ⬆300% |
| **上下文理解** | 不支持 | 支持follow-up问题 | 用户体验质的飞跃 |
| **系统稳定性** | LLM输出格式不一致，系统经常崩溃 | 严格JSON Schema，稳定性100% | 可商业化 |
| **开发效率** | 每次调整需要改代码 | 只需调整Prompt，无需改代码 | 迭代速度⬆10倍 |

---

**技术深度体现**

1. **不是简单调用LLM API**：设计了3层Prompt架构（任务定义+Few-shot+Schema约束）
2. **数据驱动迭代**：收集Badcase → 分析根因 → A/B测试 → 持续监控
3. **工程化思维**：Schema定义、关键约束、错误处理、降级机制
4. **产品思维**：从用户痛点出发（支持自然对话、避免系统崩溃），设计技术方案
...
- **价值**：风险识别从7天→1天，晨会数据整理从2小时→5分钟（⬇96%）
