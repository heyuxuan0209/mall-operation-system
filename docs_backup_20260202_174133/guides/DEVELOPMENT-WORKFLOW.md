...
3️⃣ 如果是外部文档，请提供路径：
   示例：/Users/xxx/Desktop/文档/教程.md

请回复你的选择（如："是，项目内" 或 "是，外部：/path/to/file"）
```
...
```
用户: "是，项目内"
Claude: [执行标准5步流程] → Git提交
```

**场景2：保存到外部文档**
```
用户: "是，外部：/Users/heyuxuan/Desktop/教程/新功能说明.md"
Claude: [保存文件到指定位置] → 不提交Git
```

**场景3：暂不保存**
```
用户: "否，继续开发"
Claude: 好的，继续。我会在token使用达到70%时再次提醒。
```
...
**应该保存到项目内部的情况**：
- ✅ 项目源代码（.tsx, .ts, .js等）
- ✅ 项目文档（README, CHANGELOG等）
- ✅ 配置文件
- ✅ 测试代码
- ✅ 项目相关的技术文档

**关键：先询问，再执行！永远不要假设保存位置！**
...
7. **上下文token使用超过 50%** (100K/200K) - **Claude会主动提醒**
8. **Claude主动提醒时** - **必须响应并做出选择**
...
1. ✅ 停止继续开发新功能
2. ✅ 明确告知保存决定（是/否）
3. ✅ 如果是，明确告知保存位置（项目内/外部路径）
4. ✅ 等待保存完成后再继续

**不要忽略Claude的提醒！这是防止上下文溢出的关键机制！**
